...
#Using code from Mark Newman, with permission
...
if abs(spacing1-spacing2)>1/100:
    print("The energy states are not equally spaced. The difference in spacing is",abs(spacing1-spacing2))
...
psi2pts=np.array(psi2pts,float)
#probability density
p2=0.5*h*(psi2pts[0]**2)
for n in range(1,psi2pts.size-1):
    p2+=psi2pts[n]**2
p2+=0.5*h*(psi2pts[psi2pts.size-1]**2)

#divide by the square root of the integral, will be graphed later
psi2ptsn=psi2pts/np.sqrt(p2)

#compute new integral to check it is normalized
p2norm=0.5*h*(psi2pts[0]**2)
for n in range(1,psi2pts.size-1):
    p2norm+=psi2ptsn[n]**2
p2norm+=0.5*h*(psi2pts[psi2pts.size-1]**2)


#calculate integral for psi3
psi3pts=np.array(psi3pts,float)
#probability density
p3=0.5*h*(psi3pts[0]**2)
for n in range(1,psi3pts.size-1):
    p3+=psi3pts[n]**2
p3+=0.5*h*(psi3pts[psi3pts.size-1]**2)

#divide by the square root of the integral, will be graphed later
psi3ptsn=psi3pts/np.sqrt(p3)

#compute new integral to check it is normalized
p3norm=0.5*h*(psi3pts[0]**2)
for n in range(1,psi3pts.size-1):
    p3norm+=psi3ptsn[n]**2
p3norm+=0.5*h*(psi3pts[psi3pts.size-1]**2)


#calculate integral for psi4
psi4pts=np.array(psi4pts,float)
#probability density
p4=0.5*h*(psi4pts[0]**2)
for n in range(1,psi4pts.size-1):
    p4+=psi4pts[n]**2
p4+=0.5*h*(psi4pts[psi4pts.size-1]**2)

#divide by the square root of the integral, will be graphed later
psi4ptsn=psi4pts/np.sqrt(p4)

#compute new integral to check it is normalized
p4norm=0.5*h*(psi4pts[0]**2)
for n in range(1,psi4pts.size-1):
    p4norm+=psi4ptsn[n]**2
p4norm+=0.5*h*(psi4pts[psi4pts.size-1]**2)

#generate the graph
plt.title(r"First 3 Energy States for a Quantum Anharmonic Oscillator")
plt.plot(xpoints,psi2ptsn,label=r"Ground State $E_1$")
plt.plot(xpoints,psi3ptsn,label=r"1st State $E_2$",marker='.',markersize=3)
plt.plot(xpoints,psi4ptsn,label=r"2nd State $E_3$",marker='.',markersize=0.05)
plt.xlabel(r"x (m)")
